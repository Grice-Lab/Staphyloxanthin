...
DBDBNeutrophils  = ggplot(aes(x=condition, y=ly6g_percentage_cd11b), data=flowdata_dbdb) +ylim(c(0,45))+
  ...
  geom_jitter(position=position_jitter(.1), size=2)  + theme_classic() + 
  theme(axis.title.x = element_text(face="bold",size=15),axis.title.y = element_text(face="bold",size=15),
        axis.text.y = element_text(face="bold",size=15),axis.text.x = element_text(face="bold",size=15, angle=90, vjust=.5), 
        plot.title=element_text(face="bold", size=18,hjust=.5, vjust=1.5)) + 
  ylab("Neutrophils(%) db/db Mice") +
  xlab("Condition") + 
...
ggsave(DBDBNeutrophils, file="Figures/Figure4/FigureS4A_NeutrophilsDBDB.pdf", width=6, height=5)
...
PunchCrtN = t.test(dbdb_neutrophil_punch$neutrophil_percent,dbdb_neutrophil_crtN$neutrophil_percent, pool.sd=F )
LAC_crtN = t.test(dbdb_neutrophil_crtN$neutrophil_percent,dbdb_neutrophil_LAC$neutrophil_percent, pool.sd=F)
Punch_LAC = t.test(dbdb_neutrophil_LAC$neutrophil_percent,dbdb_neutrophil_punch$neutrophil_percent, pool.sd=F)



DBDB_CrtN  = ggplot(aes(x=condition, y=neutrophil_percent), data=DBDB_Flow_CrtN) + ylim(c(0,45))+
  geom_boxplot(fill="darkorange")+
  geom_jitter(position=position_jitter(.1), size=2,color="black")  + theme_classic() + 
  theme(axis.title.x = element_text(face="bold",size=15),axis.title.y = element_text(face="bold",size=15),
        axis.text.y = element_text(face="bold",size=15),axis.text.x = element_text(face="bold",size=15, angle=90, vjust=.5), 
        plot.title=element_text(face="bold", size=18,hjust=.5, vjust=1.5)) + 
  ylab("Neutrophils(%)")  + xlab("Condition") 
...
ggsave(DBDB_CrtN, file="Figures/Figure4/Figure4E_CrtN_LAC_Neutrophils.pdf", width=6, height=5)
...
bl6_CrtN  = ggplot(aes(x=condition, y=neutrophil_percent), data=bl6_Flow_LAC) + ylim(c(0,55))+
  ...
  theme(axis.title.x = element_text(face="bold",size=15),axis.title.y = element_text(face="bold",size=15),
        axis.text.y = element_text(face="bold",size=15),axis.text.x = element_text(face="bold",size=15, angle=90, vjust=.5), 
        plot.title=element_text(face="bold", size=18,hjust=.5, vjust=1.5)) + 
  ylab("Neutrophils(%) C57BL/6 Mice") + ggtitle("Bl6 Mice") + xlab("Condition") 
...
ggsave(bl6_CrtN, file="Figures/Figure4/Bl6_crtN_Neutrophils.pdf", width=6, height=5)




ArrangedCrtComparison = grid.arrange(DBDB_CrtN,bl6_CrtN, ncol=2)
ggsave(ArrangedCrtComparison, file="Figures/Figure4/UpToDateCrtN_LAC_Neutrophils.pdf", width=12, height=10)



# Pooling additional observations of SA1088 vs. SA925 in Bl6 (Tori says this is valid to do with cell type Pcts but not cell counts)

# New 4D Previous data:
#########################
...
bl6_neutrophil_punch$Experiment="Exp2"
bl6_neutrophil_SA1088$Experiment="Exp2"
bl6_neutrophil_SA925$Experiment="Exp2"

Bl6nphil_punch$Experiment="Exp1"
Bl6nphil_SA1088$Experiment="Exp1"
Bl6nphil_SA925$Experiment = "Exp1"
...
bl6_neutrophils_clinical  = ggplot(aes(x=condition, y=neutrophil_percent), data=fullDF_bl6) + ylim(c(0,48))+ geom_boxplot(fill="darkorange")+
  geom_jitter( position=position_jitter(.1), size=2)  + theme_classic() + 
  theme(axis.title.x = element_text(face="bold",size=15),axis.title.y = element_text(face="bold",size=15),
        axis.text.y = element_text(face="bold",size=15),axis.text.x = element_text(face="bold",size=15, angle=90, vjust=.5), 
        plot.title=element_text(face="bold", size=18,hjust=.5, vjust=1.5)) + 
  ylab("Neutrophils(%) C57BL/6 Mice") + xlab("Condition") 
...
ggsave(bl6_neutrophils_clinical, file="Figures/Figure4/Figure4D_PooledExpsBl6_Clinical.pdf", width=6, height=5)
...
ggsave(ggplot(dbdbCFUs, aes(x=condition, y=log10(cfu_chromagar))) + geom_boxplot(fill="dodgerblue")+geom_jitter(height=0, width=.2) + theme_classic() + stat_compare_means(method="t.test",  comparisons = list(c("lac", "crtn")))+ylim(4,5),file="Figures/Figure4/DBDB_cfus_flow.pdf", width=5,height=4 )
